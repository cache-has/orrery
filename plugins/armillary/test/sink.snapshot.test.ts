...
interface TargetRow {
  ...
  armillary_valid_from: unknown;
  armillary_valid_to: unknown;
...
async function readAll(dir: string): Promise<TargetRow[]> {
  ...
  try {
    const reader = await conn.runAndReadAll(
      'SELECT order_id, customer, amount, refunded, armillary_scd_id, ' +
        'armillary_valid_from::VARCHAR AS armillary_valid_from, ' +
        'armillary_valid_to::VARCHAR AS armillary_valid_to, ' +
        'armillary_is_current FROM orders ORDER BY order_id, armillary_valid_from',
    );
...
describe('OrrerySink — snapshot (SCD2)', () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'armillary-orrery-snapshot-'));
  });
  ...
  it('runs four stage-diff-merge cycles and maintains SCD2 invariants', async () => {
    ...
    for (const r of rows) {
      expect(r.armillary_is_current).toBe(true);
      expect(r.armillary_valid_to).toBeNull();
      expect(r.armillary_valid_from).not.toBeNull();
      expect(typeof r.armillary_scd_id).toBe('string');
      expect((r.armillary_scd_id as string).length).toBe(32); // md5 hex
    }
    const firstRunScdIds = new Set(rows.map((r) => r.armillary_scd_id));
    ...
    for (const r of rows) {
      expect(r.armillary_is_current).toBe(true);
      expect(r.armillary_valid_to).toBeNull();
      expect(firstRunScdIds.has(r.armillary_scd_id)).toBe(true);
    }
    ...
    const closed = order2.find((r) => r.armillary_is_current === false);
    const current = order2.find((r) => r.armillary_is_current === true);
    expect(closed).toBeDefined();
    expect(current).toBeDefined();
    expect(closed!.armillary_valid_to).not.toBeNull();
    expect(current!.armillary_valid_to).toBeNull();
    expect(current!.amount).toBe(99.0);
    expect(closed!.amount).toBe(20.0);
    expect(current!.armillary_scd_id).not.toBe(closed!.armillary_scd_id);
    ...
    expect(order1[0].armillary_is_current).toBe(false);
    expect(order1[0].armillary_valid_to).not.toBeNull();
    // Orders 2 and 3 should still have exactly one current version each.
    const currentOrder2 = rows.filter((r) => r.order_id === 2 && r.armillary_is_current);
    const currentOrder3 = rows.filter((r) => r.order_id === 3 && r.armillary_is_current);
