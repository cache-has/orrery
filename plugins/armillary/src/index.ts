...
// Entry point for the Orrery Armillary sink plugin.
//
// Wires the wire-protocol session loop in `protocol.ts` to the Orrery
...
import { OrrerySink } from './sink.js';
import { runSession } from './protocol.js';

const PLUGIN_NAME = 'orrery';
...
main().catch((err: unknown) => {
  process.stderr.write(`orrery-plugin: fatal: ${(err as Error).message}\n`);
  process.exit(1);
});
